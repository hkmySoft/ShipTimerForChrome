 ...
 */
OAuth2.prototype.openAuthorizationCodePopup = function(callback, flg) {
 ...
 */
OAuth2.prototype.authorize = function(callback, flg) {
  var that = this;
  OAuth2.loadAdapter(that.adapterName, function() {
    ...
    if (!data.accessToken) {
      ...
      that.openAuthorizationCodePopup(callback, flg);
...
OAuth2.prototype.getIdToken = function() {
	var authorizationCode = this.adapter.parseAuthorizationCode(window.location.href);
	console.log(authorizationCode);
